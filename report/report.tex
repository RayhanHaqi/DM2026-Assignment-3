...
\newcommand{\filepath}[1]{\texttt{\seqsplit{#1}}}
\lstdefinestyle{cmd}{
...
\begin{tabularx}{\textwidth}{@{} >{\raggedright\arraybackslash}p{0.24\textwidth} c Y c @{}}
\toprule
...
All tree models were tuned with Optuna~\cite{akiba2019optuna} (TPESampler + MedianPruner) inside 5-fold \texttt{GroupKFold} splits by user~\cite{pedregosa2011sklearn}. This prevents samples from the same user from leaking across train/validation folds, which is essential given the user-based structure of the HAR data.

Key validation policies discovered during experimentation:

\begin{itemize}
    \item \textbf{SMOTE inside CV:} Applied within each fold before training~\cite{chawla2002smote}. Improves Optuna's ability to find parameters that balance minority-class performance.
    ...
    \item \textbf{Repeated grouped CV:} We added \filepath{model/validation.py} to run repeated \texttt{StratifiedGroupKFold} splits~\cite{pedregosa2011sklearn} with multiple random seeds. The module reports mean accuracy, fold standard deviation, worst-fold accuracy, and prediction-distribution checks before submission.
\end{itemize}

\subsection{Temporal and Label Alignment}

Each CSV file is treated as one labeled sample: the activity label applies to the \textbf{entire 300-timestep window}, not to individual timesteps. Training and test rows are keyed by file \texttt{Id} in \texttt{sample\_submission.csv}; there is no per-timestep label to align. Temporal features (segment means, deltas, jerk, energy) are computed \textbf{within} each file's window so that every feature vector remains aligned with its single class label. Cross-validation uses \texttt{GroupKFold} by \texttt{user}~\cite{pedregosa2011sklearn} so that all files from one wearer stay in the same fold, preventing user-level leakage across train and validation splits.
...
Current best (XGB targeted temporal) & \textbf{0.7691} & 0 & n/a \\
...
\textbf{Stream C: Cross-Validation Overhaul + Multi-Objective Ensemble.} The single GroupKFold-5 with fixed seed is replaced by repeated \texttt{StratifiedGroupKFold} (5 folds $\times$ 3 repeats = 15 splits)~\cite{pedregosa2011sklearn} during Optuna tuning~\cite{akiba2019optuna}. Three separate XGBoost models are tuned for different objectives: f1\_macro (current best metric), accuracy (direct Kaggle optimization), and neg\_log\_loss (probability calibration). Their test probabilities are averaged to produce a consensus prediction that balances the tendencies of each objective.
...
\textbf{Final recipe (public 0.7897):} TabPFN V3 on 91 targeted-temporal features; cached probabilities in \filepath{output/prob\_cache/tabpfn\_v3\_91f1.npz}; class multipliers $[0.80, 0.92, 1.80, 1.0, 1.80, 1.80]$; temperature $0.58$. Submission file: \filepath{submission\_tabpfn\_v3\_cal\_ft\_c08\_c1092\_b18\_t058\_20260609\_094157\_01.csv}.
...
\begin{itemize}
    ...
    \item \filepath{model/train.py}: Extended \texttt{tune\_xgboost()} and \texttt{tune\_lightgbm()} with repeated \texttt{StratifiedGroupKFold} (\texttt{n\_repeats}) and a third objective, \texttt{neg\_log\_loss}, alongside f1\_macro and accuracy.
...
\begin{enumerate}
    \item Install dependencies (downloads Kaggle data; requires \texttt{tabpfn} for TabPFN experiments):
    \begin{lstlisting}[style=cmd]
pip install -e .
    \end{lstlisting}
    \item Execute the analysis notebook:
    \begin{lstlisting}[style=cmd]
...
\begin{itemize}
    \item \textbf{File:} \filepath{submission\_tabpfn\_v3\_cal\_ft\_c08\_c1092\_b18\_t058\_20260609\_094157\_01.csv}
...
\vspace{0.5cm}
\noindent \textit{Final best submission: \filepath{submission\_tabpfn\_v3\_cal\_ft\_c08\_c1092\_b18\_t058\_20260609\_094157\_01.csv} (public 0.7897). Full experiment log: \filepath{output/SUBMISSIONS.md}.}
...
\emph{Journal of Artificial Intelligence Research}, vol.~16, pp.~321--350, 2002.
...
in \emph{Proc. KDD}, 2016, pp.~785--794.

\bibitem{ke2017lightgbm}
G.~Ke \emph{et al.},
``LightGBM: A Highly Efficient Gradient Boosting Decision Tree,''
in \emph{Proc. NIPS}, 2017.
...
in \emph{Proc. KDD}, 2019.

\bibitem{hollmann2023tabpfn}
N.~Hollmann, S.~M{\"u}ller, K.~Eggensperger, and F.~Hutter,
...
\emph{Data Mining and Knowledge Discovery}, vol.~34, no.~5, pp.~1454--1495, 2020.
